...
class AutoLearner:
    ...
    def _fit(self, x_train, y_train, rank=None, runtime_limit=None):
        ...
        def multiprocessing_with_timeout(function, list_arguments, timeout=5, verbose=False):
            n_procs = len(list_arguments)
            arr = mp.Array('f', np.full(n_procs, np.nan))
            args = [(i, item) for i, item in enumerate(list_arguments)]    

            def run_in_proc(arg, arr):
                arr[arg[0]] = function(arg[1])

            for arg in args:
                proc = mp.Process(target=run_in_proc, args=(arg, arr))
                procs.append(proc)
                proc.start()
            ...
            while time.time() - start <= timeout:
                ...
                if verbose:
                    print("Timed out, killing unfinished processes.")
                for p in procs:
                    p.terminate()
                    p.join()
        ...
        if self.selection_method is not 'random':            
            ...
            p1.close()
            p1.join()
#             sample_model_errors = [multiprocessing_with_timeout(Model.kfold_fit_validate, [m, x_train, y_train, 5])
    ...
    def fit(self, x_train, y_train, verbose=False):
        ...
        if num_points > 10000 and num_points / num_features > self.dataset_ratio_threshold:
            if verbose:
                print("Number of data points larger than 10,000. Subsampling performed.")
        ...
        if verbose:
            print("Predicted runtime: {}".format(t_predicted))
